...
function drawGraph (dataArray){
    ...
    var graph2Enter= graph2.enter()
        ...
        .on("mouseover", function(d,i){
            ...
            if (d.depth==1){tooltip2.select("#where").text(function (d){
                //console.log(nodes[i].key);
                continentName=(nodes[i].key);
                return continentName})}
            else if (d.depth==2){tooltip2.select("#where").text(function (d){
                countryName = (nodes[i].children[0].country);
                return countryName})}
            else if (d.depth==3){tooltip2.select("#where").text(function (d){
                consulateName=(nodes[i].consulate);
...
//exit
    ...
    text
        //.transition()
        //.duration(1000)
        .text(function(d) {console.log("click",d);return d.key})
        .attr("x", function(d){return d[1]})
        .attr("dx", "6") // margin
        .attr("dy", ".35em")
